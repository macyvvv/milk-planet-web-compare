...
| `proposal/branch5/shop/*/menu/index_0.html` | preserved JPEG menu comparison pages | regression source | verify only | image-menu source | local `../eventlist.js` references are inside HTML comments in the affected pages |
...
| `proposal/branch5/shop/*/eventlist.js` | captured 404 HTML responses with a `.js` suffix | stale external snapshot artifact | record and defer deletion | historical artifact | 9 files fail JS syntax; no active shop-top page resolves them |

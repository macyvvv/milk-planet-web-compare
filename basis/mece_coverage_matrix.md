...
| L-02 | Observe / scope lock | `planet-web-workflow` | `basis/README.md`、対象work packet | branch、dirty tree、比較元、対象外 | repo contract、git差分 | maintainer | PARTIAL | branch1とbranch5の「比較元／改修対象」境界が未解決 |
...
| L-10 | Operate / freshness / incident / deprecate | 未定義 | 未定義 | owner、SLA、監視、復旧、廃止記録 | 未定義 | 未定義 | GAP | 公開後運用が公開URL確認で終了している |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Q-01 | 正本・店舗固有性・設計意図 | `design-intent` + `visual-fidelity` | `DESIGN.md`、Evidence Matrix | source lock、Evidence Ledger、Swap / Flattening test | packet構造のみ | design reviewer | PARTIAL | `DESIGN.md`と将来の`design/`の関係が未決定 |
...
| Q-09 | SEO / discoverability | 未定義 | 未定義 | title、description、canonical、OGP、sitemap、robots、構造化データ | 未定義 | 未定義 | GAP | 公開サイトとしての発見性を品質領域に含めていない |
| Q-10 | Security / privacy / legal / license | 未定義 | 未定義 | 外部依存、CSP、権利、個人情報、第三者資産の確認 | 未定義 | 未定義 | GAP | 静的Webでも外部画像・フォント・予約導線の確認が必要 |
| Q-11 | Release / rollback / observability | `planet-web-workflow` | operations.md、release.md | rollback対象、公開確認、障害記録、監視結果 | CIはcontractとwhitespaceのみ | release owner unknown | PARTIAL | 公開後監視・通知・復旧演習がない |
| Q-12 | Business outcome / measurement | 未定義 | 未定義 | KPI、baseline、目標、実利用者検証 | 未定義 | 未定義 | GAP | Revenue、来店、予約、応募への接続を記録する仕組みがない |
...
| ID | Conflict / gap | Decision required | Completion condition |
| --- | --- | --- | --- |
| C-01 | `DESIGN.md`と将来の`design/` | 現行正本を`DESIGN.md`に固定するか、移行計画を作る | 正本が一つに決まり、registry・Skill・validatorが一致する |
| C-02 | branch1とbranch5 | 比較元、改修対象、公開対象をbranch・path単位で分類する | work packetとREADMEの作業境界が矛盾しない |
| C-03 | Matrix / traceability / source-map / decision log | 各文書を「横断要件」「個別証拠」「判断」「学習」に分離する | 1情報1正本、他文書は参照リンクだけになる |
| C-04 | CIの対象と検査範囲 | branch patternと必須検証を定義する | 対象PRでcontract、packet、静的品質検査が実行される |
| C-05 | 公開後責任 | owner、freshness、incident、deprecateを定義する | L-10がGAPからCOVEREDになる |
| C-06 | SEO / security / legal / KPI | Planet静的サイトの対象範囲と最小検証を決める | Q-09〜Q-12がGAPまたは明示的OUT OF SCOPEになる |
